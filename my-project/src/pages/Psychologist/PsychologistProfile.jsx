...
export default function PsychologistProfile() {
  ...
  const handleBookAppointment = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/psychologists/${id}/bookings/`, 
        {
          method: 'POST',
          headers: {
      ...
      // Refresh availability with loading state
      ...
      try {
        ...
        if (updatedResponse.ok) {
          ...
          if (updatedData.availability) {
            setAvailability(
              updatedData.availability.map(avail => ({
                id: avail.id,
                start: parseISO(avail.start),
                end: parseISO(avail.end),
                status: avail.status,
    ...
    return (
      <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-gray-600 font-medium text-sm sm:text-base">Loading profile...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        <div className="text-center">
          <div className="bg-white border border-red-200 rounded-xl p-4 sm:p-6 shadow-sm">
            ...
            <h2 className="text-red-800 font-bold text-lg sm:text-xl mb-1 sm:mb-2">Error</h2>
    ...
    return (
      <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        <div className="text-center bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-1 sm:mb-2">Profile not found</h3>
          <p className="text-gray-500 text-sm sm:text-base">The psychologist profile you're looking for doesn't exist.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid gap-4 sm:gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-6">
          ...
          <div className="flex items-start mb-3 sm:mb-6">
            ...
            <div>
              <h2 className="text-lg sm:text-2xl font-bold text-gray-900 mb-1">{psychologist.username}</h2>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full font-medium">
                  {psychologist.gender ? psychologist.gender.charAt(0).toUpperCase() + psychologist.gender.slice(1) : 'Not specified'}
                </span>
              </div>
            </div>
          </div>
          ...
          {psychologist.specializations && (
            <div className="mb-4 sm:mb-6">
              <div className="flex items-center space-x-1 sm:space-x-2 mb-2 sm:mb-3">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-gray-900">Specializations</h3>
              </div>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {psychologist.specializations.split(',').map((spec, i) => (
                  <span key={i} className="text-xs bg-green-100 text-green-800 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full font-medium">
                    {spec.trim()}
                  </span>
                ))}
              </div>
            ...
            <div className="mb-4 sm:mb-6">
              ...
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {psychologist.languages.map((lang, i) => (
                  <span key={i} className="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full font-medium">
                    {lang}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-2 sm:space-y-3">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1 sm:mb-2">Contact Information</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-6">
          <div className="flex items-center space-x-1 sm:space-x-2 mb-4 sm:mb-6">
            ...
            <h2 className="text-lg sm:text-2xl font-bold text-gray-900">Available Appointments</h2>
          </div>
              ...
              {availability.map((slot) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                      ...
                      <div>
                        <p className={`font-medium text-sm sm:text-base ${
                          slot.status === 'available' ? 'text-gray-900' : 'text-gray-600 line-through'
                        }`}>
                    ...
                    <div className={`flex items-center space-x-1 sm:space-x-2 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-medium ${
                      slot.status === 'available'
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50">
          <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 max-w-md w-full border border-gray-100">
            <div className="text-center mb-4 sm:mb-6">
              ...
              <h2 className="text-lg sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">Book Appointment</h2>
              <div className="bg-blue-50 p-2 sm:p-4 rounded-lg">
                <p className="text-blue-800 font-medium text-xs sm:text-base">{formatDate(selectedSlot.start)}</p>
